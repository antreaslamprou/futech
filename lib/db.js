...
// Reset Link
export async function createResetLink(userId) {
...
export async function getTokenDataByToken(token) {
  const client = await pool.connect();
  try {
    const res = await client.query(`SELECT * FROM password_reset_tokens WHERE token=$1`, [token]);
    const tokenData = camelcaseKeys(res.rows[0]);
    return { success: true, tokenData: tokenData};
  } catch (error) {
    console.error('Reset link token error:', error);
    throw error;
  } finally {
    client.release();
  }
...
export async function updatePassword(token, userId, password) {
  ...
  try {
    await client.query('BEGIN'); 
    const passwordRes = await client.query(`UPDATE users SET password = $1 WHERE id = $2 RETURNING *;`, [password, userId]);

    if (passwordRes.rows.length === 0) {
      throw new Error('User not found');
    }
    
    const tokenRes = await client.query(`UPDATE password_reset_tokens SET is_used = TRUE WHERE token = $1 AND user_id = $2;`, [token, userId]);
    await client.query('COMMIT');

    return camelcaseKeys(passwordRes.rows[0]);
  } catch (err) {
    await client.query('ROLLBACK'); 
    throw err;  
  } finally {
